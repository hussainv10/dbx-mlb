...
spark.createDataFrame(dbutils.fs.ls('dbfs:/user/hussain.vahanvaty@databricks.com/kinatrax/bronze/batting_motion_sequence_batting_test_checkpoint/')).display()
...
def add_row_index(df, batch_id):
  ...
  (
    ...
    .mode("append")
    .option("mergeSchema", True)
    .saveAsTable(f"{table_bronze}_test")
  )
