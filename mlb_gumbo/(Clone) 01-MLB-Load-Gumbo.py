...
# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS main.abooth_db.bronze_gumbo_data;
# MAGIC CREATE TABLE main.abooth_db.bronze_gumbo_data (
# MAGIC   data VARIANT,
...
DATA_SOURCE = "mlb_gumbo"
TABLE = "bronze_gumbo_data"

# Data Location
VOLUME_PREFIX = f"/Volumes/{CATALOG}/{SCHEMA}/{VOLUME}/{DATA_SOURCE}"
...
folder_path = f"{VOLUME_PREFIX}/year={current_run.year}/month={current_run.month}/day={current_run.day}/hour={current_run.hour}/minute={current_run.minute}"

# Create the directory structure if it doesn't exist
os.makedirs(folder_path, exist_ok=True)

# Get GamePKs
game_pks = [775300, 775294, 775296, 775297, 775298]
...
# spark.sql("drop table if exists main.abooth_db.bronze_gumbo_data")
...
query = (spark.readStream
  ...
  .writeStream
  .option("checkpointLocation", f"{VOLUME_PREFIX}/mlb_gumbo_checkpoints")
...
SELECT 
    ...
    last_update_time
FROM
    main.abooth_db.bronze_gumbo_data
""")
...
df.createOrReplaceTempView("temp_silver_gumbo_game_data")

# Use the MERGE INTO SQL command to update or insert records based on the game_pk column
spark.sql("""
MERGE INTO main.abooth_db.silver_gumbo_game_data AS target
USING temp_silver_gumbo_game_data AS source
...
# Create a temporary view from the DataFrame
df.createOrReplaceTempView("temp_silver_gumbo_pitch_data")

# Use the MERGE INTO SQL command to update or insert records based on the game_pk, at_bat_index, pitch_index column
spark.sql("""
MERGE INTO main.abooth_db.silver_gumbo_pitch_data AS target
USING temp_silver_gumbo_pitch_data AS source
ON target.game_pk = source.game_pk and 
